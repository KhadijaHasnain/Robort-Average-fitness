...
int main() {
    ...
    int averageFitness[MAX_GENERATIONS];
    ...
    // Print the average fitness scores for each generation
    std::cout << "\nAverage Fitness Scores:\n";
    for (int generation = 0; generation < MAX_GENERATIONS; ++generation) {
        std::cout << "Generation " << generation << ": " << averageFitness[generation] << std::endl;
    }
