...
void simulateTurn(Robot &robot) {
    for (int i = 0; i < NUM_GENES; ++i) {
        if (robot.energy <= 0) {
            break; // Stop simulation if energy is depleted
        }
...
int main() {
    ...
    for (int i = 0; i < POPULATION_SIZE; ++i) {
        population[i] = getRandomRobot();
    }

    int averageFitness[MAX_GENERATIONS] = {0};
    ...
    for (int generation = 0; generation < MAX_GENERATIONS; ++generation) {
        ...
        averageFitness[generation] = totalFitness / POPULATION_SIZE;

        std::cout << "Generation " << generation << " - Average Fitness: " << averageFitness[generation] << std::endl;
    }

    int overallAverageFitness = 0;
    for (int i = 0; i < MAX_GENERATIONS; ++i) {
        overallAverageFitness += averageFitness[i];
    }

    overallAverageFitness /= MAX_GENERATIONS;

    std::cout << "Overall Average Fitness: " << overallAverageFitness << std::endl;
